...
namespace SteamKit2
{
    public partial class SteamUser
    {
        ...
        public sealed class EncryptedAppTicketCallback : CallbackMsg
        {
            ...
            {
                var clientMsgProtobuf = new ClientMsgProtobuf<CMsgClientRequestEncryptedAppTicketResponse>(packetMsg);
                Result = (EResult)clientMsgProtobuf.Body.eresult;
                AppId = clientMsgProtobuf.Body.app_id;
                using var memoryStream = new MemoryStream();
                Serializer.Serialize(memoryStream, clientMsgProtobuf.Body.encrypted_app_ticket);
                Ticket = memoryStream.ToArray();
            }
        }
    }
}